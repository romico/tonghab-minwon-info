...
function writeBatLauncher(outDir) {
  // ASCII-only. Start runs node in the SAME window (foreground) — reliable on Windows.
  // Background/start-new-window caused clean exit (code=0) right after listen on some PCs.
  const batLines = [
    "@echo off",
    "setlocal EnableExtensions EnableDelayedExpansion",
    "cd /d \"%~dp0\"",
    "title Tonghab Minwon Info",
    "prompt $G",
    "",
    ...
    "echo ========================================",
    "echo.",
    "echo   1^) Start server (this window)",
    "echo   2^) Stop / kill port",
    "echo   3^) Open browser",
    "echo   4^) Exit",
    "echo.",
    "set \"CHOICE=\"",
    ...
    "call :status",
    "if not errorlevel 1 (",
    "  echo Already running. !STATUS_TEXT!",
    ...
    "del /f /q \"%~dp0data\\server.port\" >nul 2>&1",
    "start \"\" cmd /c \"timeout /t 3 /nobreak >nul & if exist \\\"%~dp0data\\server.port\\\" (for /f %%p in (%~dp0data\\server.port) do start http://%HOST%:%%p) else (start http://%HOST%:%PORT%)\"",
    "\"%NODE_EXE%\" --experimental-sqlite server.cjs",
    "set \"EC=!ERRORLEVEL!\"",
    "del /f /q \"%PID_FILE%\" >nul 2>&1",
    "echo.",
    "echo Server stopped. code=!EC!",
    "echo.",
    "pause",
    "goto menu",
    "",
    ...
    "set \"SERVER_PID=\"",
    "if exist \"%PID_FILE%\" (",
    "  for /f \"usebackq delims= \" %%A in (\"%PID_FILE%\") do set \"SERVER_PID=%%A\"",
    "  if defined SERVER_PID (",
    "    tasklist /FI \"PID eq !SERVER_PID!\" 2>nul | findstr /I /C:\"!SERVER_PID!\" >nul",
    "    if not errorlevel 1 (",
    "      set \"STATUS_TEXT=running (PID !SERVER_PID!)\"",
    "      exit /b 0",
    "    )",
    "  )",
    "  del /f /q \"%PID_FILE%\" >nul 2>&1",
    ")",
    "netstat -ano 2>nul | findstr \":%PORT%\" | findstr /I \"LISTENING\" >nul",
    "if not errorlevel 1 (",
    ...
    "  )",
    ")",
    "for /f \"tokens=*\" %%L in ('netstat -ano 2^>nul ^| findstr \":%PORT%\" ^| findstr /I \"LISTENING\"') do (",
    "  set \"_KP=\"",
    "  for %%P in (%%L) do set \"_KP=%%P\"",
    "  if defined _KP (",
    "    echo !_KP!| findstr /R \"^[0-9][0-9]*$\" >nul",
    "    if not errorlevel 1 (",
    "      echo  - port PID !_KP!",
    "      taskkill /PID !_KP! /F /T >nul 2>&1",
    "    )",
    "  )",
    ")",
    "del /f /q \"%PID_FILE%\" >nul 2>&1",
  ...
  const batBody = batLines.join("\r\n");
  const bad = [...batBody].find((ch) => ch.charCodeAt(0) > 127);
  if (bad) {
    throw new Error(`bat must be ASCII-only (U+${bad.codePointAt(0).toString(16)})`);
  }
  writeFileSync(join(outDir, "my-minwon-server.bat"), batBody, "ascii");
  for (const legacy of ["launcher-ctl.ps1", "run-server.cmd"]) {
...
■ 포트 변경 · Windows 제외 범위
  - 기본 PORT=9000 (http://127.0.0.1:9000)
  - 실행 전 set PORT=9876 후 ${launcher} 또는 스크립트 기본값 수정
  - listen EACCES(permission denied) 이고 netstat에 포트가 없어도
    Hyper-V/WSL TCP 제외 범위일 수 있음:
      netsh interface ipv4 show excludedportrange protocol=tcp
  - 상세: docs/portable-windows-port-troubleshooting.md
`,
    "utf8",
  );
}
